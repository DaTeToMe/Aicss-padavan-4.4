...
// 添加缺失的宏定义，避免未定义错误
#ifndef MIN_TTL
#define MIN_TTL 60  // 最小 TTL，例如 60 秒
#endif
#ifndef MAX_TTL
#define MAX_TTL 86400  // 最大 TTL，例如 1 天
#endif
...
static int dnsproxy(unsigned short local_port, const char* remote_addr, unsigned short remote_port, int remote_tcp)
{
    ...
    while(1) {
        ...
        // 修正：移除乱码字符，使用正确变量 current
        time(&current);
        if(current - last_clean > CACHE_CLEAN_TIME || fds == 0) {
            last_clean = current;
